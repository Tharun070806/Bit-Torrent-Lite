...
class Controller{
    ...
    */
    ...
    Controller(Mode mode, int total_pieces, int piece_length,int64_t length, std::vector<std::string>&piece_hash,std::string filename=""):
    mode(mode),
          total_pieces(total_pieces),
          piece_length(piece_length),
          length(length),
          piece_hash(piece_hash),
          filename((mode==Mode::LEECHER)? filename+"_downloaded":filename),
    ...
    std::optional<BlockRequest> next_request(const std::vector<bool>bit_field){
        ...
        for (int i = 0; i < total_pieces; i++) {
            ...
            for (int b = 0; b < num_blocks; b++) {
                ...
                if (inflight[i].find(offset)!=inflight[i].end())    continue;
    ...
    bool all_done(){
        ...
        for(int i=0;i<total_pieces;i++){
            if(piece_status[i]==2 or piece_status[i]==0){
                return false;
            }
        }
        std::cout<<"my god whole file received"<<std::endl;
        return true;
    }
    ...
    void wait_until_done() {
        ...
        cv.wait(lock, [this] {
            for (int i=0;i<total_pieces;i++)
                if (piece_status[i]!=1) return false;
            return true;
        });
    }
    ...
    */
    ...
    bool received_block(int piece_index, int block_offset, const std::string &data){
        ...
        if(data.size()!=std::min(block_length,piece_length_for(piece_index)-block_offset)){
            std::cerr<< "The block is not appropriate length "<<std::endl;
            return false;
        }


        std::cout<<"Have received/now processing a block "<<piece_index<<" "<<block_offset<<std::endl;
        ...
        for(int i=0;i<total_pieces;i++){
            std::cout<<i<<" "<<piece_status[i]<<std::endl;
            if(piece_status[i]==0 or piece_status[i]==2){
                return true;
            }
        }
